...
static	::gpk::error_t								initClient						(::gpk::SUDPClient & bestClient)										{
	...
	::gpk::SJSONFile										fileJSONConfig					= {};
	::gpk::view_const_string								fileNameJSONConfig			= "gpk_config.json";
	{	// Attempt to load config file.
		rew_if(errored(::gpk::jsonFileRead(fileJSONConfig, fileNameJSONConfig)), "Failed to load config JSON file! File not found? File name: %s.", fileNameJSONConfig.begin());
	}
	{ // attempt to load address from config file.
		::gpk::view_const_string								jsonPort							= {};
		const int32_t											indexObjectApp						= ::gpk::jsonExpressionResolve("application.bigeye", fileJSONConfig.Reader, 0, jsonPort);
		rew_if(errored(indexObjectApp), "Failed to find application node (%s) in json configuration file: '%s'", "application.bigeye", fileNameJSONConfig.begin());
		{ // 
			::gpk::view_const_string								jsonIP							= {};
			gwarn_if(errored(::gpk::jsonExpressionResolve("remote_ip", fileJSONConfig.Reader, indexObjectApp, jsonIP)), "Failed to load config from json! Last contents found: %s.", jsonIP.begin())
			else {
		...
		{ // load port from config file
			...
			jsonPort											= {};
			gwarn_if(errored(::gpk::jsonExpressionResolve("remote_port"	, fileJSONConfig.Reader, indexObjectApp, jsonPort)), "Failed to load config from json! Last contents found: %s.", jsonPort.begin()) 
			else {
				uint64_t												port								= 0;
