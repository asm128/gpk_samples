...
namespace klib {
	...
	void								drawEntityDetail					(::gpk::view_grid<char> display, ::gpk::view_grid<uint16_t> textAttributes, int32_t offsetY, int32_t offsetX, const SEntity& entity, const SEntityRecord<_TEntity> (&definitions)[_SizeDefinitions], const SEntityRecord<_TEntity> (&modifiers)[_SizeModifiers], const ::gpk::view_const_string& entityTypeName)	{
		::std::string							nameAndLevelText					= "-- " + std::string(entityTypeName.begin()) + ": ";
		lineToGridColored(display, textAttributes, COLOR_GREEN , offsetY++, offsetX, ::klib::SCREEN_LEFT, nameAndLevelText.c_str(), (uint32_t)nameAndLevelText.size());
		nameAndLevelText					= getEntityName(entity, definitions, modifiers) + " Lv. " + std::to_string(entity.Level);
		lineToGridColored(display, textAttributes, COLOR_YELLOW, ++offsetY, offsetX, ::klib::SCREEN_LEFT, nameAndLevelText.c_str(), (uint32_t)nameAndLevelText.size());
		...
		uint16_t								color;//
		sprintf_s(formattedGauge, "%i", entityPoints.Coins						);	printfToGrid(display, ++offsetY	, offsetX, ::klib::SCREEN_LEFT, formatCoins.begin()	, "Coins per turn"			, formattedGauge);	valueToGrid(textAttributes, offsetY, offsetX+23, ::klib::SCREEN_LEFT, &(color = (entityPoints.Coins >= 0) ? COLOR_ORANGE : COLOR_RED), 1, 11);
		sprintf_s(formattedGauge, "%i", entityPoints.PriceBuy / 2				);	printfToGrid(display, ++offsetY	, offsetX, ::klib::SCREEN_LEFT, formatCoins.begin()	, "Sell Price"				, formattedGauge);	valueToGrid(textAttributes, offsetY, offsetX+23, ::klib::SCREEN_LEFT, &(color = COLOR_ORANGE), 1, 11);
		sprintf_s(formattedGauge, "%i", entityPoints.CostMaintenance			);	printfToGrid(display, ++offsetY	, offsetX, ::klib::SCREEN_LEFT, formatCoins.begin()	, "Maintenance Cost"		, formattedGauge);	valueToGrid(textAttributes, offsetY, offsetX+23, ::klib::SCREEN_LEFT, &(color = COLOR_ORANGE), 1, 11);
	}
}
