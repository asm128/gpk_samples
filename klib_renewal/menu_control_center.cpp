...
int32_t drawList(::gpk::view_grid<char> display, ::gpk::view_grid<uint16_t> textAttributes, const SListItem<_TReturn> (&listItems)[_SizeList], uint32_t rowCount, int32_t offsetX, int32_t offsetY, int32_t width) {
	...
	for(uint32_t iRow = 0; iRow < rowCount; ++iRow)
		::klib::printfToGridColored(display, textAttributes, (uint16_t)listItems[iRow].Color, offsetY+iRow, offsetX, ::klib::SCREEN_LEFT, formatRow, listItems[iRow].Text.c_str());
...
int32_t drawList(::gpk::view_grid<char> display, ::gpk::view_grid<uint16_t> textAttributes, const ::gpk::array_obj<SListItem<_TReturn>>& listItems, uint32_t rowCount, int32_t offsetX, int32_t offsetY, int32_t width) {
	...
	for(uint32_t iRow = 0; iRow < rowCount; ++iRow) {
		...
		::klib::printfToGridColored(display, textAttributes, (uint16_t)listItems[iRow].Color, offsetY, offsetX, ::klib::SCREEN_LEFT, formatRow, rowText.c_str());
	}
	return 0;
}
...
int32_t drawAgentResume(::gpk::view_grid<char> display, ::gpk::view_grid<uint16_t> textAttributes, const CCharacter& agent, int32_t offsetY, int32_t offsetX, int32_t selectedAgentField) {
	...
	columnOffset = printfToGridColored(display, textAttributes, (uint16_t)finalColor, offsetY++, offsetX, ::klib::SCREEN_LEFT, " %c - %-38.38s", ::klib::ascii_gender[agent.Flags.Tech.Gender], agent.Name.c_str());
...
int32_t drawValueSlider(::gpk::view_grid<char> display, ::gpk::view_grid<uint16_t> textAttributes, int32_t offsetY, int32_t offsetX, int32_t value, int32_t labelMaxLen, const ::gpk::label& controlLabel ) {
	...
	sprintf_s(preformatted, "%i", value);								; printfToGridColored(display, textAttributes, COLOR_GREEN<<0	, offsetY, offsetX+labelMaxLen+ 4	, ::klib::SCREEN_LEFT, " %3.3s%% "	, preformatted);
...
void drawBalance(::gpk::view_grid<char> display, ::gpk::view_grid<uint16_t> textAttributes, const ::klib::SInput& frameInput, SPlayer& player, int32_t offsetX, int32_t offsetY) {
	// Budgets
...
)
{
	...
	::drawEntityDetail(display, textAttributes, offsetY, offsetX, entity, tableDefinitions, tableModifiers, entityTypeName);
...
)
{
	...
	for(uint32_t iEntity = 0, entityCount = entityContainer.Count; iEntity < entityCount; ++iEntity)
	{
		...
		printfToGridColored(display, textAttributes, colorRow, offsetY+1+iEntity, offsetX, ::klib::SCREEN_LEFT, "%12.12s %-30.30s", "", entityName.c_str());
	}
...
)
{
	...
	for(uint32_t iEntity = 0, entityCount = army.size(); iEntity < entityCount; ++iEntity) {
		...
		printfToGridColored(display, textAttributes, colorRow, offsetY+1+actualRowsDisplayed, offsetX, ::klib::SCREEN_LEFT, " %c - %-38.38s", ::klib::ascii_gender[agent.Flags.Tech.Gender], entityName.c_str());
		colorRow &= 0xF0;
...
//
int32_t drawEquipList(ENTITY_TYPE entityType, ::gpk::view_grid<char> display, ::gpk::view_grid<uint16_t> textAttributes, const SPlayer& player, int32_t offsetY, int32_t offsetX, int32_t selectedRow) {
	const ::gpk::label& labelet = ::gpk::get_value_label(entityType);

	switch(entityType) {
	case ENTITY_TYPE_CHARACTER	:	drawAgentList(display, textAttributes, player, offsetY	, offsetX + entityType, selectedRow, player.Army); break;
	case ENTITY_TYPE_PROFESSION	:	drawEquipList(display, textAttributes, offsetY + entityType, offsetX, selectedRow, labelet, player.Goods.Inventory.Profession	, definitionsProfession	, modifiersProfession	); break;
	case ENTITY_TYPE_WEAPON		:	drawEquipList(display, textAttributes, offsetY + entityType, offsetX, selectedRow, labelet, player.Goods.Inventory.Weapon		, definitionsWeapon		, modifiersWeapon		); break;
	case ENTITY_TYPE_ARMOR		:	drawEquipList(display, textAttributes, offsetY + entityType, offsetX, selectedRow, labelet, player.Goods.Inventory.Armor		, definitionsArmor		, modifiersArmor		); break;
	case ENTITY_TYPE_ACCESSORY	:	drawEquipList(display, textAttributes, offsetY + entityType, offsetX, selectedRow, labelet, player.Goods.Inventory.Accessory	, definitionsAccessory	, modifiersAccessory	); break;
	case -1:
	default:
		break;
	}
...
int32_t drawEquipDetail(ENTITY_TYPE entityType, ::gpk::view_grid<char> display, ::gpk::view_grid<uint16_t> textAttributes, const CCharacter& agent, int32_t offsetY, int32_t offsetX) {
	const ::gpk::label& labelSelectedEquip = ::gpk::get_value_label(entityType);
	switch(entityType) {
	case ENTITY_TYPE_CHARACTER	:	displayDetailedAgentSlot(display, textAttributes, offsetY, offsetX, agent, (COLOR_YELLOW<<4)| COLOR_BLUE );	break;
	case ENTITY_TYPE_PROFESSION	:	drawEquipDetail(display, textAttributes, offsetY, offsetX, labelSelectedEquip, agent.CurrentEquip.Profession	, definitionsProfession	, modifiersProfession	);	break;
	case ENTITY_TYPE_WEAPON		:	drawEquipDetail(display, textAttributes, offsetY, offsetX, labelSelectedEquip, agent.CurrentEquip.Weapon		, definitionsWeapon		, modifiersWeapon		);	break;
	case ENTITY_TYPE_ARMOR		:	drawEquipDetail(display, textAttributes, offsetY, offsetX, labelSelectedEquip, agent.CurrentEquip.Armor		, definitionsArmor		, modifiersArmor		);	break;
	case ENTITY_TYPE_ACCESSORY	:	drawEquipDetail(display, textAttributes, offsetY, offsetX, labelSelectedEquip, agent.CurrentEquip.Accessory	, definitionsAccessory	, modifiersAccessory	);	break;
	default:
		break;
	}
	return 0;
...
int32_t drawWelcomeGUI(SGame& instanceGame) {
	...
	// Squad
	printfToGridColored(display, textAttributes, COLOR_YELLOW << 4, TACTICAL_DISPLAY_POSY-2, 1, ::klib::SCREEN_LEFT, "%-43.43s", " Assigned squad:");
	...
	for(int32_t iAgent=0; iAgent < player.Squad.Size; ++iAgent) {
		...
		drawAgentResume(display, textAttributes, agent, offsetY, 1, selectedAgentField);
		...
		if(selectedEquip != -1 && selectedAgent == iAgent) {
			...
			drawEquipDetail(selectedEntityType, display, textAttributes, agent, startY, 45+1);
			...
			if(tempRow != -1) {
				...
				if(tempRow & 0x80000000) {
					...
					switch(selectedEntityType) {
					case ENTITY_TYPE_CHARACTER	:	if(player.Army[indexRow]) displayDetailedAgentSlot(display, textAttributes, startY, offsetX, *player.Army[indexRow], (COLOR_YELLOW<<4)| COLOR_BLUE);	break;
					case ENTITY_TYPE_PROFESSION	:	drawEquipDetail(display, textAttributes, startY, offsetX, labelSelectedEquip, player.Goods.Inventory.Profession	[indexRow].Entity, definitionsProfession	, modifiersProfession	);	break;
					case ENTITY_TYPE_WEAPON		:	drawEquipDetail(display, textAttributes, startY, offsetX, labelSelectedEquip, player.Goods.Inventory.Weapon		[indexRow].Entity, definitionsWeapon		, modifiersWeapon		);	break;
					case ENTITY_TYPE_ARMOR		:	drawEquipDetail(display, textAttributes, startY, offsetX, labelSelectedEquip, player.Goods.Inventory.Armor		[indexRow].Entity, definitionsArmor			, modifiersArmor		);	break;
					case ENTITY_TYPE_ACCESSORY	:	drawEquipDetail(display, textAttributes, startY, offsetX, labelSelectedEquip, player.Goods.Inventory.Accessory	[indexRow].Entity, definitionsAccessory		, modifiersAccessory	);	break;
					default:
						break;
					}
				}
	...
	if(selectedEquip != -1) {
		...
		for(int32_t iAgent=0; iAgent < player.Squad.Size; ++iAgent) {
			...
			if(selectedAgent == iAgent)
				drawEquipList((ENTITY_TYPE)selectedEquip, display, textAttributes, player, offsetY, 1, selectedRow);
		...
		for(int32_t iAgent=0; iAgent < player.Squad.Size; ++iAgent) {
			...
			if(indexEquip != -1) {
				if(selectedEquip == -1 && selectedAgent == -1)
					drawEquipDetail((ENTITY_TYPE)rowIndex, display, textAttributes, agent, startY, 45+1);
	...
	drawBalance(display, textAttributes, instanceGame.FrameInput, instanceGame.Players[PLAYER_INDEX_USER], display.metrics().x - 38, startY);
