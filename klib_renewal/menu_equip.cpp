...
bool	equipIfResearchedProfession	(::klib::SGame& instanceGame, int32_t indexAgent, int16_t selectedChoice) { ::klib::SGamePlayer& player = instanceGame.Players[::klib::PLAYER_INDEX_USER]; static const ::gpk::view_const_string modifierTypeName = "Rank"		, entityTypePlural = "Licenses"		; ::klib::CCharacter& playerAgent = *player.Tactical.Army[indexAgent];	bool bResult = equipIfResearched(instanceGame.EntityTables, selectedChoice, playerAgent, player.Inventory.Profession	, playerAgent.CurrentEquip.Profession	, player.Tactical.Research.Profession	, playerAgent.Goods.CompletedResearch.Profession		, instanceGame.EntityTables.Profession	, modifierTypeName, instanceGame.Messages.UserSuccess, instanceGame.Messages.UserError); if(bResult)instanceGame.LogSuccess(); else instanceGame.LogError(); return bResult;};
bool	equipIfResearchedWeapon		(::klib::SGame& instanceGame, int32_t indexAgent, int16_t selectedChoice) { ::klib::SGamePlayer& player = instanceGame.Players[::klib::PLAYER_INDEX_USER]; static const ::gpk::view_const_string modifierTypeName = "Science"	, entityTypePlural = "Weapons"		; ::klib::CCharacter& playerAgent = *player.Tactical.Army[indexAgent];	bool bResult = equipIfResearched(instanceGame.EntityTables, selectedChoice, playerAgent, player.Inventory.Weapon		, playerAgent.CurrentEquip.Weapon		, player.Tactical.Research.Weapon		, playerAgent.Goods.CompletedResearch.Weapon			, instanceGame.EntityTables.Weapon		, modifierTypeName, instanceGame.Messages.UserSuccess, instanceGame.Messages.UserError); if(bResult)instanceGame.LogSuccess(); else instanceGame.LogError(); return bResult;};
bool	equipIfResearchedArmor		(::klib::SGame& instanceGame, int32_t indexAgent, int16_t selectedChoice) { ::klib::SGamePlayer& player = instanceGame.Players[::klib::PLAYER_INDEX_USER]; static const ::gpk::view_const_string modifierTypeName = "Technology", entityTypePlural = "Armors"		; ::klib::CCharacter& playerAgent = *player.Tactical.Army[indexAgent];	bool bResult = equipIfResearched(instanceGame.EntityTables, selectedChoice, playerAgent, player.Inventory.Armor			, playerAgent.CurrentEquip.Armor		, player.Tactical.Research.Armor		, playerAgent.Goods.CompletedResearch.Armor				, instanceGame.EntityTables.Armor		, modifierTypeName, instanceGame.Messages.UserSuccess, instanceGame.Messages.UserError); if(bResult)instanceGame.LogSuccess(); else instanceGame.LogError(); return bResult;};
bool	equipIfResearchedAccessory	(::klib::SGame& instanceGame, int32_t indexAgent, int16_t selectedChoice) { ::klib::SGamePlayer& player = instanceGame.Players[::klib::PLAYER_INDEX_USER]; static const ::gpk::view_const_string modifierTypeName = "Crafting"	, entityTypePlural = "Accessories"	; ::klib::CCharacter& playerAgent = *player.Tactical.Army[indexAgent];	bool bResult = equipIfResearched(instanceGame.EntityTables, selectedChoice, playerAgent, player.Inventory.Accessory		, playerAgent.CurrentEquip.Accessory	, player.Tactical.Research.Accessory	, playerAgent.Goods.CompletedResearch.Accessory			, instanceGame.EntityTables.Accessory	, modifierTypeName, instanceGame.Messages.UserSuccess, instanceGame.Messages.UserError); if(bResult)instanceGame.LogSuccess(); else instanceGame.LogError(); return bResult;};
//static	bool equipIfResearchedStageProp	(SGame& instanceGame, int16_t selectedChoice) { SGamePlayer& player = instanceGame.Players[PLAYER_INDEX_USER]; static const ::gpk::view_const_string modifierTypeName = ""				, entityTypePlural = "Stage Props"	; CCharacter& playerAgent = *player.Army[player.Squad.Agents[player.Selection.PlayerUnit]];	bool bResult = equipIfResearched(selectedChoice, playerAgent, player.Goods.Inventory.StageProp	, playerAgent.CurrentEquip.StageProp	, player.Goods.CompletedResearch.StageProp	, playerAgent.Goods.CompletedResearch.StageProp	, definitionsStageProp		, modifiersStageProp	, modifierTypeName, instanceGame.UserSuccess, instanceGame.UserError); if(bResult)instanceGame.LogSuccess(); else instanceGame.LogError(); return bResult;};
//static	bool equipIfResearchedFacility	(SGame& instanceGame, int16_t selectedChoice) { SGamePlayer& player = instanceGame.Players[PLAYER_INDEX_USER]; static const ::gpk::view_const_string modifierTypeName = "Architectonics"	, entityTypePlural = "Facilities"	; CCharacter& playerAgent = *player.Army[player.Squad.Agents[player.Selection.PlayerUnit]];	bool bResult = equipIfResearched(selectedChoice, playerAgent, player.Goods.Inventory.Facility	, playerAgent.CurrentEquip.Facility		, player.Goods.CompletedResearch.Facility	, playerAgent.Goods.CompletedResearch.Facility		, definitionsFacility		, modifiersFacility		, modifierTypeName, instanceGame.UserSuccess, instanceGame.UserError); if(bResult)instanceGame.LogSuccess(); else instanceGame.LogError(); return bResult;};
//static	bool equipIfResearchedVehicle	(SGame& instanceGame, int16_t selectedChoice) { SGamePlayer& player = instanceGame.Players[PLAYER_INDEX_USER]; static const ::gpk::view_const_string modifierTypeName = "Transportation"	, entityTypePlural = "Vehicles"		; CCharacter& playerAgent = *player.Army[player.Squad.Agents[player.Selection.PlayerUnit]];	bool bResult = equipIfResearched(selectedChoice, playerAgent, player.Goods.Inventory.Vehicle	, playerAgent.CurrentEquip.Vehicle		, player.Goods.CompletedResearch.Vehicle	, playerAgent.Goods.CompletedResearch.Vehicle		, definitionsVehicle		, modifiersVehicle		, modifierTypeName, instanceGame.UserSuccess, instanceGame.UserError); if(bResult)instanceGame.LogSuccess(); else instanceGame.LogError(); return bResult;};
//			bool equipIfResearchedItems		(SGame& instanceGame, int16_t selectedChoice) { return false; } //SGamePlayer& player = instanceGame.Player; return equipIfResearched(selectedChoice, player.Army, player.Inventory.Items		, player.CurrentEquip.Items			, player.Squad, player.Selection, player.CompletedResearch.Items			, definitionsItems			, modifiersItems		, ""		, entityTypePlural, messageSuccess, messageError);};
...
#define SHOP_EXIT_VALUE ((MAX_BUY_ITEMS)*2)
	...
	case ::klib::GAME_SUBSTATE_CHARACTER	: {
		...
		selectedChoice				= (int16_t)::klib::drawMenu(menuState, display.Color.View, display.DepthStencil.begin(), menuTitle, {menuItems, itemCount}, instanceGame.FrameInput, SHOP_EXIT_VALUE, -1, 40U, false);
	}
		break;
	default:
		break;
	...
	::klib::CCharacter						* playerAgent						= 0;
	switch(instanceGame.State.Substate)
	{
	case ::klib::GAME_SUBSTATE_PROFESSION	:	if(::equipIfResearchedProfession	(instanceGame, player.Tactical.Squad.Agents[player.Tactical.Selection.PlayerUnit], selectedChoice)) { retVal = { ::klib::GAME_STATE_MENU_EQUIPMENT }; }break;
	case ::klib::GAME_SUBSTATE_WEAPON		:	if(::equipIfResearchedWeapon		(instanceGame, player.Tactical.Squad.Agents[player.Tactical.Selection.PlayerUnit], selectedChoice)) { retVal = { ::klib::GAME_STATE_MENU_EQUIPMENT }; }break;
	case ::klib::GAME_SUBSTATE_ARMOR		:	if(::equipIfResearchedArmor			(instanceGame, player.Tactical.Squad.Agents[player.Tactical.Selection.PlayerUnit], selectedChoice)) { retVal = { ::klib::GAME_STATE_MENU_EQUIPMENT }; }break;
	case ::klib::GAME_SUBSTATE_ACCESSORY	:	if(::equipIfResearchedAccessory		(instanceGame, player.Tactical.Squad.Agents[player.Tactical.Selection.PlayerUnit], selectedChoice)) { retVal = { ::klib::GAME_STATE_MENU_EQUIPMENT }; }break;
...
::klib::SGameState						drawEquip				(::klib::SGame& instanceGame, const ::klib::SGameState& returnState) {
	...
	if( player.Tactical.Selection.PlayerUnit != -1 && player.Tactical.Squad.Agents[player.Tactical.Selection.PlayerUnit] != -1) {
		...
		::klib::drawEntityDetail(display, textAttributes, offsetY, offsetX = 3, playerAgent.CurrentEquip.Profession	, instanceGame.EntityTables.Profession	, labelJob			);
		::klib::drawEntityDetail(display, textAttributes, offsetY, offsetX+=48, playerAgent.CurrentEquip.Weapon		, instanceGame.EntityTables.Weapon		, labelWeapon		);
		::klib::drawEntityDetail(display, textAttributes, offsetY, offsetX+=48, playerAgent.CurrentEquip.Armor		, instanceGame.EntityTables.Armor		, labelArmor		);
		::klib::drawEntityDetail(display, textAttributes, offsetY, offsetX+=48, playerAgent.CurrentEquip.Accessory	, instanceGame.EntityTables.Accessory	, labelAccessory	);
