...
#define SHOP_EXIT_VALUE ((MAX_BUY_ITEMS)*2)
	...
	case GAME_SUBSTATE_CHARACTER	: {
		...
		selectedChoice				= (int16_t)drawMenu(menuState, display.Screen.Color.View, display.Screen.DepthStencil.begin(), menuTitle, {menuItems, itemCount}, instanceGame.FrameInput, SHOP_EXIT_VALUE, -1, 40U, false);
	}
		break;
	default:
		break;
...
static void								drawScore						(::gpk::view_grid<char> display, int32_t offsetY, int32_t offsetX, const SCharacterScore& score)																{
	...
	sprintf_s(bufferValues, "%lli", (int64_t)score.UsedPotions			); printfToGrid(display, offsetY++, offsetX, ::klib::SCREEN_LEFT, format, "Potions Used"			, bufferValues);
	sprintf_s(bufferValues, "%lli", (int64_t)score.UsedGrenades			); printfToGrid(display, offsetY++, offsetX, ::klib::SCREEN_LEFT, format, "Grenades Used"			, bufferValues);
...
SGameState							drawEquip				(SGame& instanceGame, const SGameState& returnState) {
	...
	if(GAME_SUBSTATE_MAIN == instanceGame.State.Substate)
		actualReturnState = drawMenu(menuState, instanceGame.GlobalDisplay.Screen.Color.View, instanceGame.GlobalDisplay.Screen.DepthStencil.begin(), menuTitle, ::gpk::view_array<const ::klib::SMenuItem<::klib::SGameState>>{optionsEquip}, instanceGame.FrameInput, {GAME_STATE_MENU_SQUAD_SETUP}, instanceGame.State, 30);
	else {
