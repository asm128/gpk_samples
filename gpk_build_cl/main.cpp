...
::gpk::error_t							findLastSlash					(const ::gpk::view_const_string & path) {
	int32_t										indexOfStartOfFileName			= ::gpk::rfind('\\', path);
	if(-1 != indexOfStartOfFileName) 
		return indexOfStartOfFileName;
												indexOfStartOfFileName			= ::gpk::rfind('//', path);
	if(-1 != indexOfStartOfFileName)
		return indexOfStartOfFileName;
	return -1;
}
...
int										main							(int argc, char** argv)			{
	...
	{ // Build single configuration
		...
		int32_t										indexOfLastSlash				= ::findLastSlash(configBuild.FilenameConfig);
