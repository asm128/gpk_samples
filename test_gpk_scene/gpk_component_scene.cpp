...
static ::gpk::error_t					createFromSTL			(::gpk::SComponentScene & scene, ::gpk::SSTLFile & stlFile, ::gpk::view_const_string componentName)  {
	...
	for(uint32_t iTriangle = 0; iTriangle < stlFile.Triangles.size(); ++iTriangle) {
		...
		bufferVertices		[iTriangle * 3 + 1]	= {triangle.Triangle.B.x, triangle.Triangle.B.z, triangle.Triangle.B.y};
		bufferVertices		[iTriangle * 3 + 2]	= {triangle.Triangle.C.x, triangle.Triangle.C.z, triangle.Triangle.C.y};
		...
		if(nodeToDraw.PerFaceNormal) {
			if(nodeToDraw.VertexColor < 0) {
				...
				for(uint32_t iTriangle = 0; iTriangle < nodeVertices.size() / 3; ++iTriangle) {
					...
					::gpk::drawTriangle(target_image, triangleFinal, triangleColor);
				}
			}
			else {
			}
