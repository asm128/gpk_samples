...
::gpk::error_t			draw		(::SApplication & app)											{
	...
	::gpk::textLineDrawAlignedFixedSizeLit(target, app.TextureFontMonochrome.View, fontAtlasView.metrics(), --lineOffset, offscreenMetrics, sizeCharCell, weaponProperties[app.Game.Ships.Weapon[0].IndexProperties].Name, ::gpk::bgra{app.Framework.FrameInfo.FrameMeter.FrameNumber % 0xFFU, 0xFFU, 0, 0xFFU});
	if(app.Debugging) {
